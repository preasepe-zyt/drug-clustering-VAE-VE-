...
class LocalFeatures:
    def __init__(self, mol, onehot=False, pca=False):
        if type(mol) == str:
...
def mol2local(mols, onehot = False, pca = False):
    raw_smile = []
    t = []
    for a in mols:
       try:
    ...
    data = BatchLocalFeatures([LocalFeatures(i, onehot, pca) for i in raw_smile])
    return data, t
